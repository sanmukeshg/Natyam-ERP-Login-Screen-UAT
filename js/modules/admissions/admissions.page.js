    ...
    async enrol(row) {
        ...
        if (!selectable.length) {
            await confirm({
                ...
                message: `No active batch at this branch has a free seat, so ${row.name} cannot be placed yet. `
                    + 'Every enrolled student must belong to a batch — create one, raise a capacity, '
                    + 'or free a seat, then enrol again.',
            ...
            fields: [
                {
                    ...
                    hint: 'Required. Batches at this student\u2019s level are listed first; any active batch may be chosen.'
                },
                {
