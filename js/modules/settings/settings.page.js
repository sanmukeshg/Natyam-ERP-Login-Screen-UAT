...
import {
    listCurricula, createCurriculum, updateCurriculum, setCurriculumStatus,
    deleteCurriculum, curriculumDetail, listCurriculumLevels,
...
export default class SettingsPage extends Page {
    ...
    async dispatch(action, dataset) {
        const handlers = {
            ...
            'new-course': () => this.editCourse(null),
            'edit-course': () => this.editCourse(dataset.id),
            'structure-course': () => this.courseStructure(dataset.id),
            'toggle-course': () => this.toggleCourse(dataset.id, dataset.status),
            'delete-course': () => this.deleteCourse(dataset.id),
            'master-add': () => this.masterAdd(dataset.set),
            'master-edit': () => this.masterRename(dataset.set, dataset.value, dataset.label),
            'master-toggle': () => this.masterToggle(dataset.set, dataset.value, dataset.status),
            'master-delete': () => this.masterDelete(dataset.set, dataset.value),
            'master-move': () => this.masterMove(dataset.set, dataset.value, Number(dataset.dir)),
    ...
    async masterAdd(setName) {
        const saved = await formOverlay({
            title: `Add to ${MASTER_SETS[setName]?.label || setName}`,
            variant: 'modal', size: 'sm',
            fields: [{ name: 'label', label: 'Name', required: true }],
            onSubmit: (values) => addMasterEntry(setName, values)
        });
        if (saved) { toast.success('Added.'); await this.paint(); }
    }

    async masterRename(setName, value, label) {
        const saved = await formOverlay({
            title: 'Rename',
            variant: 'modal', size: 'sm',
            fields: [{ name: 'label', label: 'Name', required: true, value: label }],
            onSubmit: (values) => updateMasterEntry(setName, value, { label: values.label })
        });
        if (saved) { toast.success('Renamed.'); await this.paint(); }
    }

    async masterToggle(setName, value, status) {
        const next = status === 'active' ? 'inactive' : 'active';
        await setMasterEntryStatus(setName, value, next);
        toast.success(next === 'active' ? 'Activated.' : 'Deactivated — existing records keep it.');
        await this.paint();
    }

    async masterDelete(setName, value) {
        const inUse = await masterEntryUsage(setName, value);
        const ok = await confirm({
            title: 'Delete this entry?',
            message: inUse > 0
                ? `${inUse} record${inUse === 1 ? '' : 's'} still use it, so it cannot be deleted. Deactivate it instead.`
                : 'It stops being offered anywhere in the application.',
            confirmLabel: inUse > 0 ? 'Deactivate instead' : 'Delete',
            danger: true
        });
        if (!ok) return;
        try {
            if (inUse > 0) await setMasterEntryStatus(setName, value, 'inactive');
            else await deleteMasterEntry(setName, value);
            toast.success(inUse > 0 ? 'Deactivated.' : 'Deleted.');
        } catch (err) { toast.error(err.message); }
        await this.paint();
    }

    async masterMove(setName, value, direction) {
        await moveMasterEntry(setName, value, direction);
        await this.paint();
    }
    ...
    async editCourse(id) {
        const courses = await listCurricula();
        const course = id ? courses.find((c) => c.id === id) : null;
        const saved = await formOverlay({
            title: course ? `Edit ${course.name}` : 'New course',
            fields: [
                { name: 'name', label: 'Name', required: true, value: course?.name },
                { name: 'code', label: 'Code', required: true, width: 'half', value: course?.code },
                { name: 'sortOrder', label: 'Display order', type: 'number', width: 'half', value: course?.sortOrder },
                { name: 'description', label: 'Description', type: 'textarea', rows: 2, value: course?.description },
                { name: 'durationValue', label: 'Duration', type: 'number', width: 'half', value: course?.durationValue },
                { name: 'durationUnit', label: 'Unit', type: 'select', width: 'half',
                  value: course?.durationUnit || 'months', options: DURATION_UNITS }
            ],
            onSubmit: (values) => (course ? updateCurriculum(course.id, values) : createCurriculum(values))
        });
        if (saved) { toast.success(course ? 'Course updated.' : 'Course created.'); await this.paint(); }
    }

    /** The Level → Stage → Lesson editor, previously the Curriculum module. */
    async courseStructure(id) {
        const detail = await curriculumDetail(id);
        const vocabulary = await listCurriculumLevels({ includeInactive: false });

        await drawer({
            title: `${detail.name} — structure`,
            description: 'Levels come from the shared list; stages and lessons are defined here.',
            size: 'wide',
            content: html`
                <div data-role="tree">${this.structureTree(detail)}</div>
            `,
            actions: [{ label: 'Done', variant: 'secondary', value: null }],
            onMount: (body, api) => {
                const repaint = async () => {
                    const fresh = await curriculumDetail(id);
                    render(body.querySelector('[data-role="tree"]'), this.structureTree(fresh));
                };
                on(body, 'click', '[data-struct]', async (_e, target) => {
                    const { struct, nodeId, name, dir, kind } = target.dataset;
                    try {
                        if (struct === 'add-level') {
                            const used = new Set(detail.structure.levels.map((l) => l.levelId));
                            const available = vocabulary.filter((l) => !used.has(l.id));
                            if (!available.length) { toast.info('Every level is already in this course.'); return; }
                            const picked = await formOverlay({
                                title: 'Add a level', variant: 'modal', size: 'sm',
                                fields: [{ name: 'levelId', label: 'Level', type: 'select', required: true,
                                    options: available.map((l) => ({ value: l.id, label: l.name })) }],
                                onSubmit: (v) => addLevelToCurriculum(id, v.levelId)
                            });
                            if (!picked) return;
                        } else if (struct === 'remove-level') { await removeLevelFromCurriculum(id, nodeId); }
                        else if (struct === 'add-stage' || struct === 'add-lesson' || struct === 'rename') {
                            const label = struct === 'add-stage' ? 'Stage name'
                                : struct === 'add-lesson' ? 'Lesson name' : 'Name';
                            const entered = await formOverlay({
                                title: struct === 'rename' ? 'Rename' : `Add a ${struct === 'add-stage' ? 'stage' : 'lesson'}`,
                                variant: 'modal', size: 'sm',
                                fields: [{ name: 'name', label, required: true, value: name || '' }],
                                onSubmit: (v) => {
                                    if (struct === 'add-stage') return addStage(id, nodeId, { name: v.name });
                                    if (struct === 'add-lesson') return addLesson(id, nodeId, { name: v.name });
                                    return kind === 'stage'
                                        ? updateStage(id, nodeId, { name: v.name })
                                        : updateLesson(id, nodeId, { name: v.name });
                                }
                            });
                            if (!entered) return;
                        }
                        else if (struct === 'remove-stage') { await removeStage(id, nodeId); }
                        else if (struct === 'remove-lesson') { await removeLesson(id, nodeId); }
                        else if (struct === 'move') { await moveNode(id, kind, nodeId, Number(dir)); }
                        await repaint();
                    } catch (err) { toast.error(err.message); }
                });
            }
    ...
    structureTree(detail) {
        const levels = detail.structure.levels;
        return html`
            <div class="row" style="justify-content: flex-end; margin-bottom: var(--space-3);">
                <button class="btn btn-sm btn-primary" data-struct="add-level">Add level</button>
            </div>
            ${levels.length ? html`
                <div class="tree">
                        ...
                        <div class="tree-level">
                            <div class="tree-row tree-row-level">
                                ...
                                <span class="tree-actions">
                                    <button class="btn btn-sm btn-ghost" data-struct="move" data-kind="level"
                                            data-node-id="${level.id}" data-dir="-1" ${li === 0 ? 'disabled' : ''}
                                            aria-label="Move up">${raw(icon('chevron-up', { size: 14 }))}</button>
                                    <button class="btn btn-sm btn-ghost" data-struct="move" data-kind="level"
                                            data-node-id="${level.id}" data-dir="1" ${li === levels.length - 1 ? 'disabled' : ''}
                                            aria-label="Move down">${raw(icon('chevron-down', { size: 14 }))}</button>
                                    <button class="btn btn-sm btn-ghost" data-struct="add-stage" data-node-id="${level.id}">Add stage</button>
                                    <button class="btn btn-sm btn-ghost" data-struct="remove-level" data-node-id="${level.id}"
                                ...
                                ${(level.stages || []).length ? level.stages.map((stage, si) => html`
                                    <div class="tree-stage">
                                        <div class="tree-row tree-row-stage">
                                            ...
                                            <span class="tree-actions">
                                                <button class="btn btn-sm btn-ghost" data-struct="move" data-kind="stage"
                                                        data-node-id="${stage.id}" data-dir="-1" ${si === 0 ? 'disabled' : ''}
                                                        aria-label="Move up">${raw(icon('chevron-up', { size: 14 }))}</button>
                                                <button class="btn btn-sm btn-ghost" data-struct="move" data-kind="stage"
                                                        data-node-id="${stage.id}" data-dir="1" ${si === level.stages.length - 1 ? 'disabled' : ''}
                                                        aria-label="Move down">${raw(icon('chevron-down', { size: 14 }))}</button>
                                                <button class="btn btn-sm btn-ghost" data-struct="add-lesson" data-node-id="${stage.id}">Add lesson</button>
                                                <button class="btn btn-sm btn-ghost" data-struct="rename" data-kind="stage"
                                                        data-node-id="${stage.id}" data-name="${stage.name}">Rename</button>
                                                <button class="btn btn-sm btn-ghost" data-struct="remove-stage" data-node-id="${stage.id}"
                                                        aria-label="Remove stage">${raw(icon('x-circle', { size: 15 }))}</button>
                                            </span>
                                        </div>
                                        <div class="tree-children">
                                            ${(stage.lessons || []).length ? stage.lessons.map((lesson, xi) => html`
                                                <div class="tree-row tree-row-lesson">
                                                    <span>${lesson.name}</span>
                                                    <span class="tree-actions">
                                                        <button class="btn btn-sm btn-ghost" data-struct="move" data-kind="lesson"
                                                                data-node-id="${lesson.id}" data-dir="-1" ${xi === 0 ? 'disabled' : ''}
                                                                aria-label="Move up">${raw(icon('chevron-up', { size: 14 }))}</button>
                                                        <button class="btn btn-sm btn-ghost" data-struct="move" data-kind="lesson"
                                                                data-node-id="${lesson.id}" data-dir="1" ${xi === stage.lessons.length - 1 ? 'disabled' : ''}
                                                                aria-label="Move down">${raw(icon('chevron-down', { size: 14 }))}</button>
                                                        <button class="btn btn-sm btn-ghost" data-struct="rename" data-kind="lesson"
                                                                data-node-id="${lesson.id}" data-name="${lesson.name}">Rename</button>
                                                        <button class="btn btn-sm btn-ghost" data-struct="remove-lesson" data-node-id="${lesson.id}"
            ...
            ` : html`<div class="empty empty-compact"><p class="empty-text">No structure yet. Add a level to begin.</p></div>`}
        `;
    }

    async toggleCourse(id, status) {
        await setCurriculumStatus(id, status === 'active' ? 'inactive' : 'active');
        toast.success(status === 'active' ? 'Course deactivated.' : 'Course activated.');
        await this.paint();
    }
    ...
    async deleteCourse(id) {
        const ok = await confirm({
            ...
            message: 'Its structure is removed. Students assigned to it keep their records but lose the link.',
            confirmLabel: 'Delete', danger: true
        });
        if (!ok) return;
        try { await deleteCurriculum(id); toast.success('Course deleted.'); }
        catch (err) { toast.error(err.message); }
        await this.paint();
    }
     ...
     */
    /**
     * Master data. Settings is the single source of truth for the values every
     * other module offers, so each set is managed here with the same controls:
     * add, rename, reorder, deactivate and delete. Deleting is refused while
     * records still use a value — deactivating keeps history readable while
     * removing it from new use.
     */
    async curriculumPanel() {
        const sets = [
            { name: 'courses', title: 'Courses', subtitle: 'Courses of study and their Level → Stage → Lesson structure.' },
            { name: 'levels', title: 'Levels / Qualifications', subtitle: 'The single ladder a student sits on.' },
            { name: 'programTypes', title: 'Programme types', subtitle: 'Offered when scheduling a programme.' },
            { name: 'expenseCategories', title: 'Expense categories', subtitle: 'Offered when recording an expense.' }
        ];

        const [courses, levels, types, categories] = await Promise.all([
            listCurricula(),
            listMasterSet('levels'),
            listMasterSet('programTypes'),
            listMasterSet('expenseCategories')
        ]);
        const data = { levels, programTypes: types, expenseCategories: categories };
        ...
        return html`
            <div class="card">
                <div class="card-header">
                    ...
                    <p class="card-subtitle">${sets[0].subtitle}</p>
                    ${canEdit ? html`
                        <div class="card-actions">
                            <button class="btn btn-sm btn-primary" data-do="new-course">Add course</button>
                        </div>` : ''}
                </div>
                <div class="card-body card-body-flush">
                    ${courses.length ? html`
                        <div class="table-wrap"><table class="table">
                            <thead><tr>
                                <th scope="col">Course</th><th scope="col">Code</th>
                                <th scope="col">Structure</th><th scope="col">Status</th><th scope="col"></th>
                            </tr></thead>
                            <tbody>
                                ${courses.map((course) => html`
                                    <tr>
                                        <th scope="row">${course.name}</th>
                                        <td class="type-caption type-muted">${course.code}</td>
                                        <td class="type-caption type-muted">
                                            ${course.counts.levels} level${course.counts.levels === 1 ? '' : 's'} ·
                                            ${course.counts.stages} stage${course.counts.stages === 1 ? '' : 's'} ·
                                            ${course.counts.lessons} lesson${course.counts.lessons === 1 ? '' : 's'}
                                        </td>
                                        <td>${course.status === 'active'
                                            ? html`<span class="badge badge-success">Active</span>`
                                            : html`<span class="badge badge-neutral">Inactive</span>`}</td>
                                        <td class="text-right">
                                            ${canEdit ? html`
                                                <button class="btn btn-sm btn-ghost" data-do="edit-course" data-id="${course.id}">Edit</button>
                                                <button class="btn btn-sm btn-ghost" data-do="structure-course" data-id="${course.id}">Structure</button>
                                                <button class="btn btn-sm btn-ghost" data-do="toggle-course"
                                                        data-id="${course.id}" data-status="${course.status}">
                                                    ${course.status === 'active' ? 'Deactivate' : 'Activate'}</button>
                                                <button class="btn btn-sm btn-danger-quiet" data-do="delete-course" data-id="${course.id}">Delete</button>
                                            ` : ''}
                                        </td>
                                    </tr>
                                `)}
                    ...
                    ` : html`<div class="empty empty-compact"><p class="empty-text">No courses yet.</p></div>`}
                </div>
            </div>

            ${sets.slice(1).map((set) => this.masterSetCard(set, data[set.name], canEdit))}
        `;
    }

    /** One editable master-data list. */
    masterSetCard(set, entries, canEdit) {
        return html`
            <div class="card">
                <div class="card-header">
                    <h2 class="card-title">${set.title}</h2>
                    <p class="card-subtitle">${set.subtitle}</p>
                    ${canEdit ? html`
                        <div class="card-actions">
                            <button class="btn btn-sm btn-primary" data-do="master-add" data-set="${set.name}">Add</button>
                        </div>` : ''}
                </div>
                <div class="card-body card-body-flush">
                    ${entries.length ? html`
                            ...
                            <tbody>
                                ${entries.map((entry, index) => html`
                                    <tr>
                                        ...
                                        <td>${entry.status === 'active'
                                            ? html`<span class="badge badge-success">Active</span>`
                                            : html`<span class="badge badge-neutral">Inactive</span>`}</td>
                                        <td class="text-right">
                                            ${canEdit ? html`
                                                <button class="btn btn-sm btn-ghost" data-do="master-move"
                                                        data-set="${set.name}" data-value="${entry.value}" data-dir="-1"
                                                        ${index === 0 ? 'disabled' : ''} aria-label="Move up">
                                                    ${raw(icon('chevron-up', { size: 14 }))}</button>
                                                <button class="btn btn-sm btn-ghost" data-do="master-move"
                                                        data-set="${set.name}" data-value="${entry.value}" data-dir="1"
                                                        ${index === entries.length - 1 ? 'disabled' : ''} aria-label="Move down">
                                                    ${raw(icon('chevron-down', { size: 14 }))}</button>
                                                <button class="btn btn-sm btn-ghost" data-do="master-edit"
                                                        data-set="${set.name}" data-value="${entry.value}"
                                                        data-label="${entry.label}">Rename</button>
                                                <button class="btn btn-sm btn-ghost" data-do="master-toggle"
                                                        data-set="${set.name}" data-value="${entry.value}" data-status="${entry.status}">
                                                    ${entry.status === 'active' ? 'Deactivate' : 'Activate'}</button>
                                                <button class="btn btn-sm btn-danger-quiet" data-do="master-delete"
                                                        data-set="${set.name}" data-value="${entry.value}">Delete</button>
                                            ` : ''}
                                        </td>
                                    </tr>
                                `)}
                    ...
                    ` : html`<div class="empty empty-compact"><p class="empty-text">Nothing defined yet.</p></div>`}
                </div>
            </div>
        `;
    }
