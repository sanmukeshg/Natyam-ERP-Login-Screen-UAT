...
export default class StudentsPage extends Page {
    ...
    async studentFields(existing = null) {
        ...
        return [
            ...
            {
                name: 'billingFrequency', label: 'Billing frequency', type: 'select', width: 'half',
                value: existing?.billingFrequency || '',
                placeholder: 'Use fee plan default',
                options: exposedFeeFrequencies().map((f) => ({ value: f.value, label: f.label })),
                hint: 'Optional. Leave as the default to bill on the fee plan\u2019s own cycle.'
            },
            {
    ...
    feesPanel(data) {
        ...
        return html`
            <div class="grid grid-3">
                ...
                ${kpiCard('Outstanding', formatMoney(fees.outstanding), fees.billingCycle ? `billed ${fees.billingCycle.frequency.toLowerCase()} · ${fees.billingCycle.label}` : (fees.overdue ? 'includes overdue' : null))}
            </div>
