...
/**
 * Removes a course outright. Students who were following it keep their records
 * and simply lose the link, so no student data is lost when an administrator
 * clears out a course that should not have been created.
 */
export async function deleteCurriculum(id) {
    session.require(MANAGE, 'delete a course');
    const assigned = (await students$.all()).filter((s) => s.curriculumId === id);
    for (const student of assigned) {
        await students$.update(student.id, { curriculumId: null });
    }
    await curricula$.remove(id, { hard: true });
    return { unlinked: assigned.length };
}
