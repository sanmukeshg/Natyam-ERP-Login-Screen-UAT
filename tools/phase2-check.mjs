...
/* ------------------------------------------------- level vocabulary (Settings) */
{
    const { listMasterSet, addMasterEntry, deleteMasterEntry } =
        await import(`${BASE}/services/settings.service.js`);
    const before = await listMasterSet('levels');
    ok('levels are maintained as Settings master data', before.length >= 13, `${before.length}`);
    await addMasterEntry('levels', { label: 'Diploma QA' });
    ok('a level can be added from Settings',
        (await listMasterSet('levels')).some((e) => e.label === 'Diploma QA'));
    await deleteMasterEntry('levels', 'diploma-qa');
    ok('an unused level can be removed',
        !(await listMasterSet('levels')).some((e) => e.label === 'Diploma QA'));
}
...
{
    // The standalone Curriculum module was retired into Settings → Curriculum.
    const settingsMod = await import(`${BASE}/modules/settings/settings.page.js`);
    const SettingsPage = settingsMod.default;
    ...
    await page.render(container);
    await settle();
    const tabKeys = [...container.querySelectorAll('[data-tab]')].map((b) => b.dataset.tab);
    ok('Settings offers a Curriculum tab', tabKeys.includes('curriculum'), tabKeys.join(','));

    const panel = await page.curriculumPanel();
    ok('the Curriculum panel builds', !!panel);
}
